...
use crate::service::account::{AccountService, RateLimitDisposition};
...
impl GatewayService {
    ...
    async fn handle_request_inner(
        ...
        if path.starts_with("/v1/messages") {
            log_request_diagnostics(&path, &query, &headers, &body_map, client_type);
        }
        ...
        log_rewritten_request_diagnostics(
            &path,
            &query,
            &final_headers,
            &final_body,
        ...
        let resp = self
            ...
            let disposition = if self.quarantine_on_429.load(Ordering::Relaxed) {
                match self.account_svc.handle_rate_limit(&account).await {
            ...
            if matches!(disposition, Some(RateLimitDisposition::PureRateLimit)) {
                warn!(
                    "account {} hit pure rate limit, returning 429 without failover",
                    account.id
                );
                return Ok(resp);
            }
    ...
    async fn forward_request(
        ...
        if status_code >= 400 {
            ...
            let body_bytes = resp.bytes().await.map_err(|e| {
                warn!(
                    "failed to read upstream error body for account {}: {}",
                    account.id, e
                );
                AppError::BadGateway("failed to read upstream error body".into())
            })?;
            info!(
                "upstream error diagnostics: account_id={} status={} body={}",
                account.id,
                status_code,
                truncate_body(&body_bytes, 4096)
            );
...
fn log_request_diagnostics(
    ...
    let model = body
        ...
        .unwrap_or_default()
        .to_string();
    let system_kind = match body.get("system") {
    ...
    let first_role = body
        ...
        .unwrap_or_default()
        .to_string();
    let first_content_kind = match body
        .get("messages")
        .and_then(|v| v.as_array())
        .and_then(|arr| arr.first())
    ...
    let metadata_user_id_prefix = body
        .get("metadata")
    ...
    info!(
        "request diagnostics: client_type={:?} path={} query={} model={} anthropic_beta={} system_kind={} messages_len={} first_role={} first_content_kind={} metadata_user_id_prefix={} top_keys={}",
        client_type,
        path,
        query,
        model,
        ...
        first_role,
        first_content_kind,
        metadata_user_id_prefix,
        top_keys
    );
}

fn log_rewritten_request_diagnostics(
    path: &str,
    query: &str,
    headers: &std::collections::HashMap<String, String>,
    body: &[u8],
    ...
    let model = body_json
        ...
        .unwrap_or_default()
        .to_string();
    let metadata_user_id_prefix = body_json
        .get("metadata")
        .and_then(|v| v.as_object())
        .and_then(|m| m.get("user_id"))
        .and_then(|v| v.as_str())
    ...
    info!(
        "rewritten request diagnostics: account_id={} client_type={:?} path={} query={} model={} anthropic_beta={} auth_style={} metadata_user_id_prefix={} top_keys={}",
        account_id,
        client_type,
        path,
        query,
